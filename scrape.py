...
class Scrape:
    ...
    def scrapeAlbumInfo(self, url):
        ...
        try:
            ...
            driver.get(url)
            time.sleep(5)
            button = driver.find_element(By.CSS_SELECTOR, 'button[data-testid="subnavicons-index"]')
            button.click()
            time.sleep(15)
        ...
        for album in albumElements:
            ...
            try:
                ...
                if albumName not in processedAlbums:
                    processedAlbums[albumName] = {'rank': rank, 'albumName': albumName.strip(), 'artist': artist.strip()}
                    rank -= 1
            except ValueError:
                continue
    ...
    def enrichAlbums(self, albums):
        ...
        for item in albums.values():
            releaseDate, genre = self.spotify.getAlbumInfo(item['albumName'], item['artist'])
            enrichedAlbums.append(Album(item["albumName"], item["artist"], genre, releaseDate, item['rank']))
